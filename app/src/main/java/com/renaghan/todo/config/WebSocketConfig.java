...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
  ...
  private ReactorNettyTcpClient<byte[]> getCustomTcpClientWithoutSSLSupport() {
    return new ReactorNettyTcpClient<>(
        configurer ->
            configurer.host(this.messageBrokerEndpoint.host).port(this.messageBrokerEndpoint.port),
  ...
  private static class Endpoint {
    ...
    static Endpoint fromEndpointString(String endpoint) {
      ...
      if (hostAndPortMatcher.matches()) {
        host = hostAndPortMatcher.group(1).replace("stomp+ssl://", ""); // see
        // https://stackoverflow.com/questions/49964647/spring-websockets-activemq-convertandsendtouser
      ...
      if (!(hostAndPortMatcher.matches() || failoverURIMatcher.matches())) {
        throw new IllegalStateException(
            String.format(
                "Invalid endpoint string (must either consist of hostname and port or a failover URI): %s",
                endpoint));
      }
