...
/**
 * Creates an ECS service on top of a {@link Network}. Loads Docker images from a {@link
 * DockerImageSource} and places them into ECS task. Creates a log group for each ECS task. Creates
 * a target group for the ECS tasks that is attached to the load balancer from the {@link Network}.
 */
class ServiceStack extends Construct {
  ...
  ServiceStack(
    ...
    List<CfnTargetGroup.TargetGroupAttributeProperty> stickySessionConfiguration =
        Arrays.asList(
    ...
    CfnTargetGroup targetGroup =
        CfnTargetGroup.Builder.create(this, "targetGroup")
    ...
    CfnListenerRule.ActionProperty actionProperty =
        CfnListenerRule.ActionProperty.builder()
            .targetGroupArn(targetGroup.getRef())
            .type("forward")
            .build();

    CfnListenerRule.RuleConditionProperty condition =
    ...
    if (httpsListenerArn.isPresent()) {
      CfnListenerRule httpsListenerRule =
          CfnListenerRule.Builder.create(this, "httpsListenerRule")
      ...
      CfnCondition httpsListenerRuleCondition =
          CfnCondition.Builder.create(this, "httpsListenerRuleCondition")
    ...
    CfnListenerRule httpListenerRule =
        CfnListenerRule.Builder.create(this, "httpListenerRule")
            .actions(singletonList(actionProperty))
            .conditions(singletonList(condition))
    ...
    Role ecsTaskExecutionRole =
        Role.Builder.create(this, "ecsTaskExecutionRole")
    ...
    CfnTaskDefinition taskDefinition =
        CfnTaskDefinition.Builder.create(this, "taskDefinition")
            // skipped family
    ...
    CfnSecurityGroup ecsSecurityGroup =
        CfnSecurityGroup.Builder.create(this, "ecsSecurityGroup")
            .vpcId(networkOutputParameters.getVpcId())
            .groupDescription("SecurityGroup for the ECS containers")
            .build();

    // allow ECS containers to access each other
    CfnSecurityGroupIngress ecsIngressFromSelf =
    ...
    CfnSecurityGroupIngress ecsIngressFromLoadbalancer =
    ...
    allowIngressFromEcs(
        serviceInputParameters.securityGroupIdsToGrantIngressFromEcs, ecsSecurityGroup);

    CfnService service =
        CfnService.Builder.create(this, "ecsService")
    ...
    // Adding an explicit dependency from the service to the listeners to avoid "has no load
    // balancer associated" error
    // (see
  ...
  private void allowIngressFromEcs(
      List<String> securityGroupIds, CfnSecurityGroup ecsSecurityGroup) {
    int i = 1;
    for (String securityGroupId : securityGroupIds) {
      CfnSecurityGroupIngress ingress =
  ...
  public static class ServiceInputParameters {
    ...
    /**
     * Knobs and dials you can configure to run a Docker image in an ECS service. The default values
     * are set in a way to work out of the box with a Spring Boot application.
     *
     * @param dockerImageSource the source from where to load the Docker image that we want to
     *     deploy.
     * @param securityGroupIdsToGrantIngressFromEcs Ids of the security groups that the ECS
     *     containers should be granted access to.
     * @param environmentVariables the environment variables provided to the Java runtime within the
     *     Docker containers.
     */
     ...
     */
    public ServiceInputParameters(
        DockerImageSource dockerImageSource, Map<String, String> environmentVariables) {
      this.dockerImageSource = dockerImageSource;
      this.environmentVariables = environmentVariables;
    ...
    /**
     * The number of consecutive successful health checks after which an instance is declared
     * healthy.
     *
     * <p>Default: 2.
     */
    ...
    /**
     * The number of consecutive unsuccessful health checks after which an instance is declared
     * unhealthy.
     *
     * <p>Default: 8.
     */
    ...
    /**
     * The number of CPU units allocated to each instance of the application. See <a
     * href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html">the
     * docs</a> for a table of valid values.
     *
     * <p>Default: 256 (0.25 CPUs).
     */
    ...
    /**
     * The memory allocated to each instance of the application in megabytes. See <a
     * href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html">the
     * docs</a> for a table of valid values.
     *
     * <p>Default: 512.
    ...
    /**
     * The maximum percentage in relation to the desired instances that may be running at the same
     * time (for example during deployments).
     *
     * <p>Default: 200.
     */
    ...
    /**
     * The minimum percentage in relation to the desired instances that must be running at the same
     * time (for example during deployments).
     *
     * <p>Default: 50.
     */
    public ServiceInputParameters withMinimumHealthyInstancesPercent(
        int minimumHealthyInstancesPercent) {
      this.minimumHealthyInstancesPercent = minimumHealthyInstancesPercent;
      return this;
    }

    /**
     * The list of PolicyStatement objects that define which operations this service can perform on
     * other AWS resources (for example ALLOW sqs:GetQueueUrl for all SQS queues).
     *
     * <p>Default: none (empty list).
     */
    public ServiceInputParameters withTaskRolePolicyStatements(
        List<PolicyStatement> taskRolePolicyStatements) {
    ...
    /**
     * The format of the date time used in log entries. The awslogs driver will use this pattern to
     * extract the timestamp from a log event and also to distinguish between multiple multi-line
     * log events.
     *
     * <p>Default: %Y-%m-%dT%H:%M:%S.%f%z (to work with JSON formatted logs created with <a
     * href="https://github.com/osiegmar/logback-awslogs-json-encoder">awslogs JSON Encoder</a>).
     *
     * <p>See also: <a
     * href="https://docs.docker.com/config/containers/logging/awslogs/#awslogs-datetime-format">awslogs
     * driver</a>
     */
    ...
    /**
     * The priority for the HTTP listener of the loadbalancer. The priority of two listeners must
     * not be the same so you need to choose different priorities for different services.
     */
